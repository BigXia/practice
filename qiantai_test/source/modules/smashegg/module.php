...
class SmasheggModule extends WeModule {
public $tablename = 'smashegg_reply';
	...
	public function ruleDeleted($rid) {
		...
		//删除fans
		pdo_delete('smashegg_fans', array('rid' => $rid));
	...
	public function doManage() {
		...
		if (!empty($list)) {
			foreach($list as &$item) {
				...
				$smashegg = pdo_fetch("SELECT fansnum, viewnum,starttime,endtime,isshow,is_default FROM ".tablename('smashegg_reply')." WHERE rid = :rid ", array(':rid' => $item['id']));
				$item['fansnum']=$smashegg['fansnum'];
				$item['viewnum']=$smashegg['viewnum'];
				$item['starttime']=date('Y/m/d H:i',$smashegg['starttime']);
				$item['endtime']=date('Y/m/d H:i',$smashegg['endtime']);
				$item['is_default']=$bigwheel['is_default'];
				
				$nowtime=time();
				if($smashegg['starttime']>$nowtime){
					$item['status']='<span class="label label-red">未开始</span>';
					$item['show']=1;
				}elseif($smashegg['endtime']<$nowtime){
					$item['status']='<span class="label label-blue">已结束</span>';
					$item['show']=0;
				}else{
					if($smashegg['isshow']==1){
	...
	public function dodelete(){
		...
		pdo_delete('smashegg_reply',array('rid' => $rid));
		message('规则操作成功！', create_url('site/module', array('do' => 'manage', 'name' => 'smashegg')), 'success');			
	}
	...
	public function doaddcard(){
		...
		if($_GPC['action']=='save'){
			...
			$insert=array(
				...
				'no_win_info'=>$_GPC['no_win_info'],
				'win_info'=>$_GPC['win_info'],
			...
			$temp = pdo_insert('smashegg_reply', $insert);
			if($temp==false){
				$this->message('抱歉，刚才添加的数据失败！','', -1);              
			}else{
				$this->message('砸金蛋添加数据成功！', create_url('site/module', array('do' => 'manage', 'name' => 'smashegg')), 0);      
			}		
		}
	...
	public function doUpdateCard (){
		...
		if($_GPC['action']=='update'){
			...
			$insert['starttime']=strtotime($starttime);
			$insert['endtime']=strtotime($endtime);
			$temp = pdo_update('smashegg_reply', $insert,array('id'=>$id,'rid'=>$rid));
			if($temp==false){
				$this->message('抱歉，刚才修改的数据失败！','', -1);              
			}else{
				$this->message('砸金蛋修改数据成功！', create_url('site/module', array('do' => 'manage', 'name' => 'smashegg')), 0);      
			}		
		} 
	...
	public function dosetshow(){
		...
		$temp = pdo_update('smashegg_reply',array('isshow'=>$_GPC['isshow']),array('rid'=>$rid));
		if($temp==false){
			message('抱歉，刚才操作数据失败！','', 'error');              
		}else{
			message('状态设置成功！', create_url('site/module', array('do' => 'manage', 'name' => 'smashegg')), 'success');    
		}	
	}
	public function dochangedefault(){
 		global $_GPC;
		pdo_update('smashegg_reply',array('is_default'=>$_GPC['ck']),array('rid'=>$_GPC['id']));
 	}	
	public function dosetstatus(){
		...
		if($temp==false){
			...
			message('状态设置成功！', create_url('site/module', array('do' => 'awardlist', 'name' => 'smashegg','rid'=>$_GPC['rid'])), 'success');    
		}		
	}
	public function dogetphone(){
		...
		$fans=$_GPC['fans'];
		//
		$tel=pdo_fetchcolumn ("SELECT tel FROM ".tablename('smashegg_fans')." WHERE rid = ".$rid." and  from_user='".$fans."'");						
		if($tel==false){
			echo '没有登记';
		}else{
			echo $tel;
	...
	public function setkeyword($_kewords,$rid=0,$_module='smashegg'){
		global $_W;
		if($rid==0){
