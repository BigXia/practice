	...
	<form id="payform" action="" method="post" class="form-horizontal form" onsubmit="return validate();">
		...
		<table class="tb">
			<tr>
				...
				<td>
					<label class="radio">
						<input type="radio" name="delivery[switch]" value="true" <?php  if($delivery['switch']) { ?> checked="checked"<?php  } ?>/>
						开启
					</label>
					<label class="radio">
						<input type="radio" name="delivery[switch]" value="false" <?php  if(empty($delivery['switch'])) { ?> checked="checked"<?php  } ?>/>
						关闭
					</label>
		...
		<table class="tb">
			<tr>
				...
				<td>
					...
					<div class="alert alert-block">
						<p>微擎0.52支持微信支付接口，注意你的微擎访问地址一定不要写错了，这里我们用访问地址代替下面说明中出现的链接，申请微信支付的接口说明如下：</p>
						<br>
						<h5>JS API网页支付参数</h5>
		...
		<?php  if(empty($wechat['version']) && !empty($wechat['signkey'])) { ?>
			<?php  $wechat['version'] = 1;?>
		<?php  } ?>
		<table class="tb wechat-params<?php  if(empty($wechat['switch'])) { ?> hide<?php  } ?>">
			<tr>
				<th>接口类型</th>
				<td>
					<label class="radio inline">
						<input type="radio" name="wechat[version]" value="1" <?php  if($wechat['version'] == 1) { ?> checked="checked"<?php  } ?>/>
						旧版
					</label>
					<label class="radio inline">
						<input type="radio" name="wechat[version]" value="2" <?php  if($wechat['version'] == 2 || empty($wechat['version'])) { ?> checked="checked"<?php  } ?>/>
						新版(2014年9月之后申请的)
					</label>
			...
			<tbody id="wechat-params-version-1" <?php  if($wechat['version'] == 2 || empty($wechat['version'])) { ?> style="display:none;"<?php  } ?>>
			<tr>
			...
			<tbody id="wechat-params-version-2" <?php  if($wechat['version'] == 1) { ?> style="display:none;"<?php  } ?>>
				<tr>
					<th><label for="">微信支付商户号<br />(MchId)</label></th>
					<td>
						<input type="text" name="wechat[mchid]" class="span6" value="<?php  echo $wechat['mchid'];?>" autocomplete="off"/>
			...
			<tr>
				...
				<td>
					<input type="text" name="wechat[signkey]" class="span6" value="<?php  if(empty($wechat['signkey'])) { ?><?php  echo random(32);?><?php  } else { ?><?php  echo $wechat['signkey'];?><?php  } ?>" autocomplete="off"/>
